...
def newPaintingButton(app, canvas):
    font = 'Times 15'
    canvas.create_rectangle(app.width//2 - 100, 225, app.width//2 + 100, 275)
    canvas.create_text(app.width//2, 250, text='new painting', font=font)

def openPaintingButton(app, canvas):
    font = 'Times 15'
    canvas.create_rectangle(app.width//2 - 100, 300, app.width//2 + 100, 350)
...
def paintMode_getSnapshot(app):
    app._showRootWindow()
    x0 = app._root.winfo_rootx() + app._canvas.winfo_rootx()
    y0 = app._root.winfo_rooty() + app._canvas.winfo_rooty()
    x1 = app._root.winfo_rootx() + app._canvas.winfo_width()
    y1 = app._root.winfo_rooty() + app._canvas.winfo_height()
    x2 = app._canvas.winfo_screenwidth()
    y2 = app._canvas.winfo_screenheight()
    result = ImageGrabber.grab((x0, y0, x0+x2, y0+y2))
    return result
...
def paintMode_saveSnapshot(app):
        ...
        if (path):
            ...
            app._deferredMethodCall(afterId='saveSnapshot', afterDelay=0, afterFn=lambda:paintMode_getSnapshot(app).save(path))
